...
class Fourier:
  ...
  def plot_spectrum(self, interactive=False):
    ...
    if interactive:
      self.trace = go.Scatter(x=self.frequencies, y=self.amplitude(), mode='lines')
    ...
    # When the argument interactive is set to False:
      ...
      plt.semilogx(self.frequencies, savgol_filter(self.amplitude(),631,3))
      plt.xlim([20,20000])
      plt.xticks([1000*2**i for i in range(-5,5)], [str(int(1000*2**i)) for i in range(-5,5)])
      plt.title('Spectrum')
  ...
  def plot_time_frequency(self, t_ylabel="Amplitude", f_ylabel="Amplitude",
    ...
    self.time_trace = go.Scatter(x=self.time_axis, y=self.signal, mode='lines')
    ...
    self.freq_trace = go.Scatter(x=self.frequencies, y=self.amplitude(), mode='lines')
