...
class Filterbank():
    ...
    def _valid_bands(self, fs, bands, bandsize):
        ...
        fc = [band*np.power(2, 1/bandsize)
              for band in bands_safe]  # upper cut freq for each band
        assert fc[-1] < (fs/2)
        self.fc = fc
        self.fs = fs
    ...
    def _gen_filters(self, order, f_length, power):
        ...
        if power:
            ir2Bands = np.real(np.fft.ifft(
                np.square(np.abs(np.fft.fft(irBands, axis=0))), axis=0))
        else:
            ir2Bands = np.real(np.fft.ifft(
                np.abs(np.abs(np.fft.fft(irBands, axis=0))), axis=0))

        ir2Bands = np.concatenate(
            (ir2Bands[nbins:(2 * nbins), :], ir2Bands[0:nbins, :]), axis=0)
